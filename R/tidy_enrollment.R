...
tidy_enr <- function(df) {
  ...
  invariants <- c(
    "end_year", "type",
    "corporation_id", "school_id",
    "corporation_name", "school_name"
  )
  ...
  result <- result %>%
    dplyr::mutate(aggregation_flag = dplyr::case_when(
      !is.na(corporation_id) & corporation_id != "" &
        !is.na(school_id) & school_id != "" ~ "campus",
      !is.na(corporation_id) & corporation_id != "" ~ "district",
      TRUE ~ "state"
    ))
...
#'
#' Adds boolean flags to identify state, corporation, and school level records.
#'
...
#' table(tidy_data$is_state, tidy_data$is_corporation, tidy_data$is_school)
#' }
id_enr_aggs <- function(df) {
  df |>
    dplyr::mutate(
      ...
      # Corporation level: Type == "Corporation"
      is_corporation = type == "Corporation",
...
enr_grade_aggs <- function(df) {
  ...
  group_vars <- c(
    ...
    "corporation_name", "school_name",
    "subgroup",
    "is_state", "is_corporation", "is_school"
  )
