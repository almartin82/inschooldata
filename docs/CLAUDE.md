...
**Gender File Merged Excel Headers**: The gender files have merged cells
in the Excel header causing `readxl` to misparse column names as `_1`,
`_2` instead of `CORP_ID`, `CORP_NAME`. The package handles this by: 1.
Detecting when first column is `_1` instead of `CORP_ID` 2. Renaming
columns appropriately 3. Removing the sub-header row containing
“Female”/“Male” labels

Without this fix, joins fall back to YEAR-only matching, causing a
cartesian product (418 × 419 = 175,142 rows instead of 418).
...
0 errors, 0 warnings - \[ \] `pytest tests/test_pyinschooldata.py` — all
tests pass - \[ \]
