...
test_that("Indiana DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Indiana DOE URL and verify HTTP 200")
})
...
test_that("Can download Indiana enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Indiana enrollment file", {
  skip_if_offline()
...
test_that("inschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- inschooldata::get_available_years()
  ...
  tryCatch({
    raw <- inschooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- inschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- inschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- inschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- inschooldata::fetch_enr(test_year, tidy = TRUE)
