...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ind.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyinschooldata as ind
        assert hasattr(ind, 'fetch_enr')
        assert hasattr(ind, 'fetch_enr_multi')
        assert hasattr(ind, 'tidy_enr')
        assert hasattr(ind, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyinschooldata as ind
        assert hasattr(ind, '__version__')
        assert isinstance(ind.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ind.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = ind.fetch_enr(years['max_year'])
        # Total enrollment in Indiana varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Indiana should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ind.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ind.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyinschooldata as ind
        years = get_test_years()
        df_single = ind.fetch_enr(years['max_year'])
        df_multi = ind.fetch_enr_multi([years['max_year']])
